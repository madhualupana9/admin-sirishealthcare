        ...
        <div class="bg-white overflow-hidden shadow-xl rounded-xl border border-gray-100 card-hover">
            <div class="p-8">
                ...
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
                    <div>
                        <h2 class="text-3xl font-bold text-gray-800">Specialties Management</h2>
                        <p class="mt-2 text-gray-600">Manage and review medical specialties</p>
                    </div>
                    <div class="mt-4 md:mt-0">
                        <a href="{{ route('specialties.create') }}" class="inline-flex items-center px-4 py-2.5 border border-transparent text-sm font-medium rounded-xl shadow-sm text-white bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all duration-150">
                                ...
                                <tr class="row-hover">
                                    <td class="px-6 py-5">
                                        <div class="flex items-center">
                                            @if($specialty->logo_image)
                                                <div class="flex-shrink-0 h-10 w-10 rounded-full overflow-hidden">
                                                    <img class="h-full w-full object-cover" src="{{ asset('storage/'.$specialty->logo_image) }}" alt="{{ $specialty->name }}">
                                                </div>
                                            @else
                                    ...
                                    <td class="px-6 py-5">
                                        <div class="text-sm text-gray-900">{{ $specialty->slug }}</div>
                                    </td>
                                    <td class="px-6 py-5">
                                    ...
                                    <td class="px-6 py-5 whitespace-nowrap text-sm font-medium">
                                        <div class="flex space-x-3">
                                            <a href="{{ route('specialties.edit', $specialty) }}" class="text-blue-600 hover:text-blue-900 action-btn inline-flex items-center">
                                                <svg class="h-4 w-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                                </svg>
                                                Edit
                                            </a>
                                            <button wire:click="deleteSpecialty('{{ $specialty->id }}')" class="text-red-600 hover:text-red-900 action-btn inline-flex items-center" onclick="return confirm('Are you sure?')">
                                                <svg class="h-4 w-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
